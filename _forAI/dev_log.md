...
**시간**: 2026-07-22 13:00 ~ 14:42 +09:00 (Korea Standard Time)
...
**사람이 띄운 창에 붙는 실검증 (14:35~14:42)**

처음에는 검증용 창을 내가 새로 띄웠는데, 사용자가 "제가 띄운 창이 있습니다. 거기에 붙어서
테스트할 줄 알았네요"라고 지적했다. 정당한 지적이다 — 이 기능의 요점이 **이미 떠 있는 창에
붙는 것**이다. 다시 사용자가 띄운 창(pid 59316)에 붙어 양방향을 확인했다.

- **사람 → AI**: 사용자가 창에서 "로컬 Chrome 시작"을 누르자 `running` False→True,
  상태 "중지됨"→"로컬 DevTools 실행 중", `owns_chrome` False→True로 읽혔고 새 로그 6줄이
  그대로 보였다.
- **AI → 사람**: `gui_tool_stop`을 호출하자 창의 상태가 "중지됨"으로 바뀌고 로그에
  "실행 중인 DevTools를 종료합니다."가 붙었으며 9222 프로필 Chrome 프로세스가 **8 → 0**이 됐다.

부수 확인 두 가지. 죽은 창이 남긴 엔드포인트 파일(pid 57444)을 `read_endpoints()`가 스스로
치우고 살아 있는 것만 돌려줬다. 그리고 사용자 창이 9222를 물고 있는 동안 `cleanup`을 돌렸을 때
**그 세션을 건너뛰었다** — 소유자 프로세스가 살아 있었기 때문이며, "두 번째 창이 첫 번째 창의
Chrome을 죽이지 않는다"는 규칙이 실제 상황에서 작동한 것이다.

**미확인**: 실제 Claude Code 세션에서 `gui-tool` 도구가 `/mcp`에 뜨는지는 세션 재시작이 필요해
아직 확인하지 않았다. 브리지 자체가 MCP stdio로 정상 동작하는 것은 위에서 확인했다.

---

### 2026-07-22 (2) — Chrome 9222 점유의 진짜 원인 규명, `_forAI` 문서 동기화

**시간**: 2026-07-22 11:25 ~ 12:40 +09:00 (Korea Standard Time)
...
**시간**: 2026-07-21 19:00 ~ 2026-07-22 이른 시각 +09:00 (Korea Standard Time).
날짜를 넘겨 진행했다. 중간에 사용량 한도로 한 번 끊겼다가 재개했으므로 정확한 종료 시각은
기록하지 않는다. 실측 타임스탬프 근거는 `%TEMP%`의 프로파일 폴더와 프로세스 생성 시각이다.
